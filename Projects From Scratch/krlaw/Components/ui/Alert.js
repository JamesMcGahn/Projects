...
function Alert({ message, type }) {
    ...
    return (
        <React.Fragment>
            {
                display ?
                    <div className={`${type === 'success' ? 'bg-green-100 border border-green-400 text-green' : 'bg-red-100 border border-red-400 text-red'} 
                    px-4 py-3 rounded relative`} role="alert">
                        <strong className="font-bold">{message}</strong>
                        <span className="absolute top-0 bottom-0 right-0 px-4 py-3" onClick={() => setDisplay(false)}>
