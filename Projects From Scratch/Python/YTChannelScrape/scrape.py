...
def run_webdriver(url):
    ...
    try:
        ...
        if '404' in driver.title:
            raise ValueError('Cannot find that youtube channel. Please check the url structure or channel name')
        else:
...
def scrape_html(filename):
    ...
    for video in all_videos:
        vid = {
            ...
            "Length": video.find(id='thumbnail').find(id='text').get_text("",strip=True),
        }
...
def start():
    ...
    if url_choice.lower() == 'quit': return
    ...
    try:
        source = run_webdriver(
        url_format[url_choice]
        )
